...
**核心逻辑：**  
小于 2 一定不是质数。然后从 2 开始尝试找因数，只需要检查到 √n，因为如果 n 是合数，它至少有一个因数 ≤ √n。

- `%` = 取余数
- `n % i == 0` = n 能被 i 整除 → 找到因数 → 不是质数
- `return` = 返回结果 + 立刻结束函数
- 检查到 √n 就够了
- 循环条件必须包含平方根，所以要用 `<=`
- 否则像 `49 = 7 × 7` 会漏掉 7
...
> 从 2 开始找因数，只查到 √n；找到能整除的就不是质数，全都找不到才是质数。
...
`2 到 n → is_prime → 是质数 → print_number → 打印`
...
- `printf` 打印
- 修改 global variable（全局变量）

这些叫 **side effect（副作用）**。

### `counter`

`counter` 必须是 global variable（全局变量），因为需要跨多次函数调用记住已经打印了几个数字。
...
# A3 · Sieve of Eratosthenes（埃拉托斯特尼筛法）

**核心思想：**

> 不逐个试除，而是把所有确定的合数划掉，最后没被划掉的就是质数。
...
> 把字符串一个字符一个字符读出来，把字符对应的数值写进 int 数组，同时统计复制了多少个字符。
...
如果想观察一个 int 内部的四个 byte，就要按 `char*` 一个 byte 一个 byte 地看。

---

# A5 · Memory Layout（内存布局）
...
> 没有显式初始化 / 初始为 0 的 global variable
...
# A5 · 为什么写一个 int 会覆盖 4 个 char

因为：

`int = 4 bytes`

如果 pointer 是 `int*`，那么给 `*p` 写一个整数，就会从当前地址开始一次写 4 bytes。
...
因为 param 和 m 是两个不同变量。

如果函数里面再：

`gv = param`

那么 global variable `gv` 会真的变成 9。

### 记忆

> 改 param ≠ 改 m  
> 改 global variable = 真正修改全局变量
...
- `char` 是 **1 byte**，不是 1 bit
- `arr[i] = 0` = 暂时没被标记为合数，不是立刻证明是质数
- `'\0'` = 字符串结束
- `'\n'` = 换行
- `NULL` = 空指针
- `src` 不是字符，而是“指向 char 的 pointer”
- `dst` 本来就是 `int*`
- `(*count)++` = 值 +1
- `count++` = 地址移动
- cast 不改变地址，只改变解释方式
- `cp = cs` 不复制字符串，只保存 `cs[0]` 的地址
- `void` = 不返回值，不代表什么都不做
