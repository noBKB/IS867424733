...
int previous = 0;        // 保存“上一个质数”。一开始还没有找到质数，所以先设成 0。
int sum = 0;             // 保存所有相邻质数之间距离的总和。一开始总和是 0。
int count = 0;           // 保存一共计算了多少个“距离”。一开始还没有距离，所以是 0。

for(i = 2; i <= limit; i++){   // 从 2 开始，一直检查到 n，逐个看哪些数字是质数。

    if(arr[i] == 0){            // arr[i] == 0 表示这个数最后没有被标记成合数，所以它是质数。

        if(previous != 0){      // 如果 previous 不是 0，说明前面已经找到过一个质数了，
                                // 那么现在“上一个质数”和“当前质数”之间就可以计算距离。

            sum += (int)i - previous;
                                // 当前质数减去上一个质数，就是这两个相邻质数之间的距离。
                                // 然后把这个距离加到 sum 里面。
                                // 例如 previous=7，当前 i=11，那么距离就是 11-7=4。

            count++;            // 每成功计算出一个距离，就把距离数量加 1。
        }

        previous = (int)i;      // 把当前这个质数保存成“上一个质数”。
                                // 这样下次遇到新的质数时，就可以拿它来计算新的距离。
    }
}

if(count > 0){                  // 如果至少算出过一个距离，才可以计算平均值。
                                // 这样可以避免 count=0 时发生除以 0。

    double average = (double)sum / count;
                                // 平均值 = 距离总和 / 距离数量。
                                // 把 sum 转成 double，是为了得到小数结果。
                                // 例如 9 / 4 应该得到 2.25，而不是整数 2。
